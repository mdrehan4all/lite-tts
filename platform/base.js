...
class SayPlatformBase {
  ...
  speak (text, voice, speed, callback) {
    ...
    if (!text) {
      return setImmediate(() => {
        callback(new TypeError('liteTTS.speak(): must provide text parameter'))
      })
    }
    ...
    this.child.addListener('exit', (code, signal) => {
      if (code === null || signal !== null) {
        return callback(new Error(`liteTTS.speak(): could not talk, had an error [code: ${code}] [signal: ${signal}]`))
      }
  ...
  export (text, voice, speed, filename, callback) {
    ...
    if (!text) {
      return setImmediate(() => {
        callback(new TypeError('liteTTS.export(): must provide text parameter'))
      })
    }

    if (!filename) {
      return setImmediate(() => {
        callback(new TypeError('liteTTS.export(): must provide filename parameter'))
      })
    }
    ...
    this.child.addListener('exit', (code, signal) => {
      if (code === null || signal !== null) {
        return callback(new Error(`liteTTS.export(): could not talk, had an error [code: ${code}] [signal: ${signal}]`))
      }
  ...
  stop (callback) {
    ...
    if (!this.child) {
      return setImmediate(() => {
        callback(new Error('liteTTS.stop(): no speech to kill'))
      })
    }
  ...
  getInstalledVoices (callback) {
    ...
    this.child.addListener('exit', (code, signal) => {
      if (code === null || signal !== null) {
        return callback(new Error(`liteTTS.getInstalledVoices(): could not get installed voices, had an error [code: ${code}] [signal: ${signal}]`))
      }
      if (voices.length > 0) {
        voices = voices.split('\r\n')
